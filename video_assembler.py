...
레이아웃 (9cm x 16cm 기준, 1cm = 120px):
  0px    ┌─────────────┐
         │  검정 바     │  444px
         │  [제목 24pt] │  (상단 바 중앙 y=222)
  444px  ├─────────────┤
         │             │
         │  사진 영역   │  1032px
         │             │
  1476px ├─────────────┤
         │  [자막 18pt] │  (하단 바 중앙 y=1698)
         │  검정 바     │  444px
  1920px └─────────────┘
"""
import subprocess
...
BAR_H         = 444
PHOTO_Y       = BAR_H
PHOTO_H       = HEIGHT - BAR_H * 2   # 1032
GAP           = 60

TITLE_SIZE    = 32   # 24pt
CAPTION_SIZE  = 24   # 18pt

TOP_BAR_CY    = BAR_H // 2                    # 222
BOTTOM_BAR_CY = PHOTO_Y + PHOTO_H + BAR_H // 2  # 1698

TITLE_MARGIN_V   = TOP_BAR_CY - TITLE_SIZE // 2      # 206
CAPTION_MARGIN_V = HEIGHT - BOTTOM_BAR_CY - CAPTION_SIZE // 2  # 210
...
def assemble_video(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if image_path is None and pexels_key:
            stock_video = _fetch_pexels_video(
                script.get("keywords", []), pexels_key, tmp, duration
            )
...
def _render(script, tts_path, ass_path, output_path, duration, image_path, stock_video):
    ...
    if image_path and image_path.exists():
        fg = (
            ...
            "[wp]ass={ass}[vout]"
        ).format(W=WIDTH, H=HEIGHT, PH=PHOTO_H, PY=PHOTO_Y, bg=photo_bg, ass=ass_path)
        ...
        fg = (
            ...
            "[wp]ass={ass}[vout]"
        ).format(W=WIDTH, H=HEIGHT, PH=PHOTO_H, PY=PHOTO_Y, ass=ass_path)
...
def _write_ass_captions(path, hook, body, closer, duration):
    ...
    lines = [
        ...
        f"Style: Title,Arial,{TITLE_SIZE},&H00FFFFFF,&H000000FF,&H00000000,"
        f"&H00000000,-1,0,0,0,100,100,2,0,1,3,0,8,60,60,{TITLE_MARGIN_V},1",
        f"Style: Caption,Arial,{CAPTION_SIZE},&H00FFFFFF,&H000000FF,&H00000000,"
        f"&H00000000,0,0,0,0,100,100,1,0,1,3,0,2,60,60,{CAPTION_MARGIN_V},1",
        f"Style: Closer,Arial,{CAPTION_SIZE},&H00FFFF00,&H000000FF,&H00000000,"
        f"&H00000000,-1,0,0,0,100,100,1,0,1,3,0,2,60,60,{CAPTION_MARGIN_V},1",
        "",
        "[Events]",
